SELECT *
FROM DUAL; -- 테이블과 아무런 관계 없는 애를 사용할 때

WHERE STATUS LIKE "Sh%" -> Sh로 시작하는 애 가져오기
"%Sh%"
...
FROM EMPLOYEES) T; -- T는 별칭


-- TRIM(문자열/공백 제거)
-- LTRIM/RTIRM
-- 제거할 문자를 입력하면 문자 제거, 입력하지 않으면 공백 제거
SELECT  
    TRIM(LEADING 'A' FROM 'AAB   ') AS C1, -- 앞에 A 있으면 A 제거
    TRIM(LEADING FROM 'AAB   ') AS C2, -- 앞에서부터 공백 제거
    TRIM(TRAILING 'B' FROM 'AAB  ') AS C3, -- 뒤에 B 있으면 B 제거
    TRIM(TRAILING FROM 'AAB   ') AS C4, -- 뒤에서부터 공백 제거
    TRIM(BOTH 'A' FROM 'AAB   ') AS C5, -- 양쪽에 A 있으면 A 제거
    TRIM(BOTH FROM 'AAB   ') AS C6, -- 양쪽 공백 제거
    TRIM('A' FROM 'AAB   ') AS C7, -- BOTH와 똑같다(defalut값)
    TRIM('AAB   ') AS C8
FROM DUAL; -- 단일행으로 출력하기

출력값: 'B   ', 'AAB   ', 'AAB   ', 'AAB', 'B   ', 'AAB', 'B   ', 'AAB'
